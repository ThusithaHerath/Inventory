...
if (isset($_GET['remove_itm']) && isset($_GET['product_sub_barcode']) && isset($_GET['return_invoice_id'])) {

    $product_sub_barcode = $_GET['product_sub_barcode'];
    $return_invoice_id = $_GET['return_invoice_id'];

    $sql = "DELETE FROM return_items WHERE itm_barcode_id ='". $product_sub_barcode ."'";
...
if (isset($_GET['product_sub_barcode']) && isset($_GET['return_invoice_id']) && !isset($_GET['remove_itm'])) {
    ...
    $return_invoice_id = $_GET['return_invoice_id'];
        ...
        $bill_id =  $list_result['sale_list_invoice'];


        $sql = "SELECT * FROM return_invoices WHERE return_invoice_id=".$return_invoice_id;
        ...
        if (mysqli_num_rows($check_qury) > 0) {
            ...
            $sql = "INSERT INTO return_invoices (return_invoice_id, bill_id) VALUES (" . $return_invoice_id ." , ". $bill_id. ")";
            mysqli_query($conn, $sql);
...
?></td>
      <td><?php
    echo $product_result['product_sub_name'];
?></td>
      <td align="right"><?php
    echo number_format($main_product_result['product_main_price'], 2);
?></td>
      <td align="right"><?php
...
?></td>
    ...
    echo $return_invoice_id;
?>','<?php
    echo $product_sub_barcode;
?>');">Remove</button></td>
    </tr>
<?php
}
