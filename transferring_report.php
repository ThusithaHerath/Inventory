...
<head>
    ...
    <title>Product Transferring Report | Dashboard</title>
                ...
                <div class="content-header">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="mr-auto">
                            ...
                            <h3 class="page-title">Product Transferring Report</h3>
...
<table>
  <tbody>
    <tr>
      <td>Start Date:<input name="start_date" type="datetime-local" required class="form-control" id="start_date" value="<?php if(isset($_POST['start_date'])){echo $_POST['start_date'];} ?>"></td>
      <td>End Date:<input name="end_date" type="datetime-local" required class="form-control" id="end_date" value="<?php if(isset($_POST['end_date'])){echo $_POST['end_date'];} ?>"></td>
...
<button type="button" class="btn btn-sm btn-dark" onClick="printDiv('print_aria');">Print Product Report</button>

<div id="print_aria">

<div class="print-title">
<img src="images/bbill_logo.png" style="height: 80px;"><br>
<strong>Product Report</strong><br>
...
if(isset($_POST['filter_bt'])){
        ...
        SELECT * FROM product_transferring WHERE id IN (SELECT MIN(id) FROM product_transferring GROUP BY transfer_key) WHERE date BETWEEN '$_POST[start_date]' AND '$_POST[end_date]' ORDER by id DESC";
}else{
    $sql = "SELECT * FROM product_transferring WHERE id IN (SELECT MIN(id) FROM product_transferring GROUP BY transfer_key)";
}

//echo $sql;
	
$no_count=0;
$list_qury1=mysqli_query($connation,$sql);


while($list_result=mysqli_fetch_array($list_qury1)){
$no_count++;

$date = $list_result['date'];
$barcode = $list_result['barcode'];
$transfer_key = $list_result['transfer_key'];

$old_institute_id = $list_result['old_institute_id'];
$sql = "SELECT * FROM thlocation WHERE lid = '".$old_institute_id."'";

$list_qury=mysqli_query($connation,$sql);
$result=mysqli_fetch_array($list_qury);
$old_institute = $result['lname'];

$new_institute_id = $list_result['new_institute_id'];
$sql = "SELECT * FROM thlocation WHERE lid = '".$new_institute_id."'";
$list_qury=mysqli_query($connation,$sql);
$result=mysqli_fetch_array($list_qury);
$new_institute = $result['lname'];



$user_id = $list_result['transferred_by'];
$sql = "SELECT * FROM thusers WHERE user_id = '".$user_id."'";
$list_qury=mysqli_query($connation,$sql);
$result=mysqli_fetch_array($list_qury);
...
<td><?php echo $date; ?></td>
<td ><?php echo $old_institute; ?></td>
<td ><?php echo $new_institute; ?></td>
<td ><?php echo $username; ?></td>
<td ><a href="transferring_item_report.php?transfer_key=<?php echo $transfer_key; ?>">View</a></td>
