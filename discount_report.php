...
if(isset($_POST['filter_btn'])){
	if($_POST['mc']=="All"){
		header("location:discount_report.php");
	}
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Discount Report | Dashboard</title>
...
<button type="button" class="btn btn-sm btn-dark" onClick="printDiv('print_aria');">Print Discount Report</button>

<div id="print_aria">

<div class="print-title">
<img src="images/bbill_logo.png" style="height: 80px;"><br>
<strong>Discount Report</strong><br>
...
$amount=0;
if(isset($_POST['filter_btn'])){
$qury=mysqli_query($connation,"SELECT sp.product_sub_main_warp,sp.product_sub_barcode,mp.product_main_price,sp.product_sub_discount,((mp.product_main_price)-(mp.product_main_price/100*sp.product_sub_discount)) AS real_price,mc.product_main_text
FROM product_sub_barcode sp INNER JOIN product_main_barcode mp ON sp.product_sub_norep=mp.product_main_norep
INNER JOIN product_main_category mc ON mp.product_main_cat=mc.product_main_id
WHERE product_sub_status IN ('1','2') AND sp.product_sub_discount>0 AND mp.product_main_cat='$_POST[mc]'");
}
else{
$qury=mysqli_query($connation,"SELECT sp.product_sub_main_warp,sp.product_sub_barcode,mp.product_main_price,sp.product_sub_discount,((mp.product_main_price)-(mp.product_main_price/100*sp.product_sub_discount)) AS real_price,mc.product_main_text
FROM product_sub_barcode sp INNER JOIN product_main_barcode mp ON sp.product_sub_norep=mp.product_main_norep
INNER JOIN product_main_category mc ON mp.product_main_cat=mc.product_main_id
WHERE product_sub_status IN ('1','2') AND sp.product_sub_discount>0");
}
while($resalt=mysqli_fetch_assoc($qury)){
$amount+=$resalt['real_price'];
?>
    <tr>
      <td><?php echo $resalt['product_main_text']." (".$resalt['product_sub_main_warp'].")"; ?></td>
      <td><?php echo $resalt['product_sub_barcode']; ?></td>
      <td align="right"><?php echo number_format((float)$resalt['product_main_price'],2); ?></td>
      <td align="right"><?php echo $resalt['product_sub_discount']; ?>%</td>
      <td align="right"><?php echo number_format((float)$resalt['real_price'],2); ?></td>
    </tr>								
<?php } ?>
    <tr>
